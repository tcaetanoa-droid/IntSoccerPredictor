...
Run: `pytest tests/test_montecarlo.py::test_resolve_meta_path_falls_back_to_this_checkouts_data_folder -v` Expected: FAIL with `ImportError: cannot import name 'resolve_meta_path'`
...
Run: `pytest -q` Expected: 98 passed (the new test and `test_full_run_matches_the_numbers_in_docs_reports`, which failed before, both pass). The full-run test takes about a minute because `context` converts 10.4 M rows to strings.
...
Run: `pytest tests/test_backtest_scores.py -v` Expected: FAIL with `ImportError: cannot import name 'scores'`
...
Run: `pytest tests/test_backtest_scores.py -v` Expected: 5 passed
...
Run: `pytest tests/test_backtest.py -v` Expected: FAIL with `ImportError: cannot import name 'forecasts'`
...
Run: `pytest tests/test_backtest.py -v` Expected: 6 passed (the engine comparison takes a few seconds)
...
Run: `pytest tests/test_backtest.py -v` Expected: the six earlier tests pass; the five new ones FAIL with `AttributeError: module ... has no attribute 'run_counts'` (and so on)
...
Run: `pytest tests/test_backtest.py -v` Expected: 11 passed
...
Run: `pytest tests/test_backtest.py -v` Expected: FAIL with `ImportError: cannot import name 'fates'`
...
Run: `pytest tests/test_backtest.py -v` Expected: 16 passed
...
Run: `pytest tests/test_backtest.py -v` Expected: FAIL with `ImportError: cannot import name 'build_backtest'`
...
Run: `pytest tests/test_backtest.py -v && ruff check --select F,E src/intsoccer/backtest tests/test_backtest.py tests/test_backtest_scores.py` Expected: 18 passed; ruff reports nothing

- [ ] **Step 6: Run the real thing once**

Run: `intsoccer backtest --run output/wc2026 --site` Expected: about two minutes (the string conversion in `context`), then the printed lines, `output/wc2026/backtest/` with four files, and `site/data/wc2026/backtest.json`. Keep the terminal output: Task 7 pins these numbers. Confirm `sparse pairings` is not printed (count 0).
...
Run: `pytest -q` Expected: all passed (about 118 tests; the two full-run modules take a few minutes together). Paste the last lines of the output into the task report.
...
- Project layout: replace `  backtest/    not started` with `  backtest/    scores.py (Brier, log-loss, RPS, skill), forecasts.py (day-of, the runs' frequencies, baselines), fates.py (real fates, ladders, hits, calibration), build.py (output/<name>/backtest/, site JSON); the record is docs/BACKTEST.md` and `  cli.py       intsoccer fetch | snapshot | fit | simulate | report` gains ` | backtest`.
...
`docs/ROADMAP.md` row 12: replace the Status cell `todo (main goal)` with `done 22 Sep 2026 (the seed-2026 run scored against the real 104 matches and 48 fates: match Brier and log-loss for the day-of and the runs' forecasts against a shrug and plain Elo, fate-ladder RPS 0.0816 vs the structural shrug's 0.1208, the three hits, five-bin calibration; \`intsoccer backtest\`, record in docs/BACKTEST.md; spec docs/superpowers/specs/2026-09-22-backtest-design.md; a re-simulation from the real round of 32 deferred as 12b)` and the row's Component cell to `2026 World Cup: transcribe groups/bracket/results to YAML, snapshot ratings at 2026-06-10, run the sim, score it (Brier / log-loss, RPS, calibration)`. Also update the component note **12 Backtest metrics** at the bottom to: `**12 Backtest.** Done as specified in docs/BACKTEST.md: two forecasts per match (day-of, the runs' frequencies) against two baselines (shrug, plain Elo), the fate ladders by RPS against a structural shrug, the three hits, the reach-the-round calibration. Approach A, scoring the stored run; approach B, a re-simulation from the real round of 32 with day-of ratings, is the possible 12b.`
